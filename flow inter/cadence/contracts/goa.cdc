...
pub contract goa: FungibleToken {
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, CollectionPublic {
        ...
        destroy() {
            goa.totalSupply = goa.totalSupply - self.balance
        }
    }
    pub resource Admin {
    ...
    pub resource Minter {
        pub fun mintToken(amount: UFix64): @FungibleToken.Vault {
            goa.totalSupply = goa.totalSupply + amount
            return <-create Vault(balance: amount)
        }
    }
    ...
    init() {
        ...
        self.account.link<&goa.Minter>(/public/Minter, target: /storage/MinterStorage)
